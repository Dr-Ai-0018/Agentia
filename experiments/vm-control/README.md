...
- 已补最小可运行真实检查器
- 2026-06-07 已完成 `jade` 真实采样验收
- 宿主视角与 guest 视角的数据已能合并输出
- 当前 `jade` 采样结果表明：
  - `1 vCPU`
  - `2GiB` 内存
  - `12GiB` 磁盘
  - guest 内可观测到 `1716 MiB` 可用内存
  - 根分区使用率 `7%`
  - 当前无资源申请信号
...
- 这一层已经能稳定回答“我现在这具身体是什么状态”。
- 输出同时包含宿主配额、guest 体感、关键目录、运行服务与压力判断，可以直接作为后续 resident 自查链路的输入。
- 在当前健康样本上，系统不会无端触发资源申请，说明阈值至少没有明显失真。
