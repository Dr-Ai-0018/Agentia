...
func main() {
	...
	case "inventory":
		...
		printJSON(out)
	case "host-inspect":
		out, err := app.RunHostInspect(*limit)
		if err != nil {
			exitf("%v", err)
		}
		printJSON(out)
	case "host-inspect-cached":
		out, err := app.RunHostInspectFromSnapshot(*limit)
		if err != nil {
			exitf("%v", err)
		}
		printJSON(out)
	case "host-inspect-summary":
		out, err := app.RunHostInspectSummary(*limit)
		if err != nil {
			exitf("%v", err)
		}
		printJSON(out)
	case "host-inspect-summary-cached":
		out, err := app.RunHostInspectSummaryFromSnapshot(*limit)
		if err != nil {
			exitf("%v", err)
		}
		printJSON(out)
	case "host-decision-assist":
		out, err := app.RunHostDecisionAssist(*limit)
		if err != nil {
			exitf("%v", err)
		}
		printJSON(out)
	case "host-decision-assist-cached":
		out, err := app.RunHostDecisionAssistFromSnapshot(*limit)
		if err != nil {
			exitf("%v", err)
		}
		printJSON(out)
	case "host-maintenance-draft":
		out, err := app.RunHostMaintenanceDraft(*limit, *apply)
		if err != nil {
			exitf("%v", err)
		}
		printJSON(out)
	case "host-maintenance-draft-cached":
		out, err := app.RunHostMaintenanceDraftFromSnapshot(*limit, *apply)
		if err != nil {
			exitf("%v", err)
		}
		printJSON(out)
	case "v0-readiness":
		out, err := app.RunV0Readiness(*limit)
		if err != nil {
			exitf("%v", err)
		}
		printJSON(out)
	case "v0-readiness-cached":
		out, err := app.RunV0ReadinessFromSnapshot(*limit)
		if err != nil {
			exitf("%v", err)
		}
		printJSON(out)
	case "v0-runbook":
		...
		printJSON(out)
	case "v0-acceptance":
		out, err := app.RunV0Acceptance(*limit)
		if err != nil {
			exitf("%v", err)
		}
		printJSON(out)
	case "v0-acceptance-cached":
		out, err := app.RunV0AcceptanceFromSnapshot(*limit)
		if err != nil {
			exitf("%v", err)
		}
		printJSON(out)
	case "v0-acceptance-evidence":
		out, err := app.RunV0AcceptanceEvidence(broker.V0AcceptanceEvidenceInput{
			...
			Operator: *operator,
			Summary:  *summary,
			Evidence: splitEvidenceBody(*body),
			Command:  *reason,
			Apply:    *apply,
		}, time.Now().UTC())
		if err != nil {
		...
		printJSON(out)
	case "v0-acceptance-evidence-list":
		out, err := app.RunV0AcceptanceEvidenceList(*limit, time.Now().UTC())
		if err != nil {
			exitf("%v", err)
		}
		printJSON(out)
	...
	case "checkpoint-cleanup":
		out, err := host.CleanupResidentCheckpoints(broker.CheckpointCleanupInput{
		...
		printJSON(out)
	case "memory-compact":
		out, err := app.RunMemoryCompact(*residentID, *apply)
		if err != nil {
			exitf("%v", err)
		}
		printJSON(out)
	case "memory-maintenance":
		out := app.RunMemoryMaintenanceSummary()
		printJSON(out)
	...
	case "memory-lifecycle-safe-apply":
		...
		printJSON(out)
	case "memory-auto-maintain":
		out, err := app.RunMemoryAutoMaintain(*residentID, *apply)
		if err != nil {
			exitf("%v", err)
		}
		printJSON(out)
	...
	case "recover-all":
		...
		out, err := app.RunRecoverAllToNow(time.Now().UTC(), mode)
		if err != nil {
			exitf("%v", err)
		}
		printJSON(out)
	...
	case "host-intervention":
		...
		printJSON(out)
	case "host-resolve-intervention":
		if *interventionID == "" {
			exitf("intervention-id is required for host-resolve-intervention mode")
		}
		out, err := host.ResolveHostIntervention(broker.HostInterventionResolveInput{
	...
	case "ticket-complete-maintenance":
		...
		printJSON(out)
	case "ticket-start-maintenance":
		if *messageID == "" {
			exitf("message-id is required for ticket-start-maintenance mode")
		}
		if *residentID == "" {
			exitf("resident is required for ticket-start-maintenance mode")
		}
		out, err := host.StartResourceMaintenance(broker.ResourceMaintenanceStartInput{
			TicketID:       *messageID,
			Resident:       *residentID,
			Resource:       *resource,
			Amount:         *amount,
...
func splitEvidenceBody(body string) []string {
	var out []string
