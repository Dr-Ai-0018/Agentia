...
func (r *Runner) Run(profile ResidentProfile, duration time.Duration, outDir string, verbose bool, resetResident bool) (FinalReport, error) {
	...
	if resetResident {
		if _, err := r.brokerApp.RunReset(profile.Name, started); err != nil {
	...
	for {
		...
		prepared, err := r.preflightCheck(profile, state, started.Add(time.Duration(round)*time.Minute))
		if err != nil {
		...
		brokerLog, err := r.settleUsageWithBroker(profile, result, started.Add(time.Duration(round)*time.Minute), runtimeguard.CallKindWork, tokenledger.ActivityNormalWork)
		if err != nil {
...
func (r *Runner) preflightCheck(profile ResidentProfile, state loopState, startedAt time.Time) (*brokerstate.PreparedAdmission, error) {
	spec := preflightSpec(profile, state, startedAt)
	prepared, err := r.brokerApp.RunPrepareSpec(profile.Name, spec)
	if err != nil {
		return nil, err
	}
	return &prepared, nil
...
func (r *Runner) settleUsageWithBroker(profile ResidentProfile, result openai.StreamResult, startedAt time.Time, kind runtimeguard.CallKind, activity tokenledger.ActivityType) (*BrokerUsageLog, error) {
	...
	resp, err := r.brokerApp.RunAdmitSpec(profile.Name, spec, true)
	if err != nil {
		return nil, err
	}
...
func (r *Runner) runAcceptance(profile ResidentProfile, history []openai.Message, verbose bool) (string, *BrokerUsageLog, error) {
	...
	brokerLog, err := r.settleUsageWithBroker(profile, result, time.Now().UTC(), runtimeguard.CallKindWork, tokenledger.ActivityNormalWork)
	if err != nil {
